public class Test {
	public static void main(String[] args) {
		...
		correctString = "";
		// Because we are not storing 0 values in the matrix the toString should reflect
		// an "empty" (all 0) matrix.
		System.out.println("toString is correct: " + correctString.equals(myTest.toString()));
		myTest.addElement(0, 1, 3);
		myTest.addElement(0, 1, 0);

		correctString = "";
		// Note that adding 0 to the matrix overwrites the data at that position to 0 as
		// defined in the interface description//Because we are not storing 0 values, we
		// can remove the element at that position.
		System.out.println("toString is correct: " + correctString.equals(myTest.toString()));
		myTest.addElement(0, 0, 16);
		myTest.addElement(0, 1, 4);
		...
		SparseInterface test3 = test1.addMatrices(test2);
		// should return null
		try {
		...
		SparseInterface test4 = test1.multiplyMatrices(test2);
		// should work
		System.out.println("test4: " + test4.toString());

		SparseInterface test5 = test2.multiplyMatrices(test1);
		// should return null
		try {
